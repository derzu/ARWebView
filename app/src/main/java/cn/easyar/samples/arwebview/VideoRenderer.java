...
public class VideoRenderer
{
    ...
    private static Bitmap textureWeb=null;
    ...
    public void render(Matrix44F projectionMatrix, Matrix44F cameraview, Vec2F size)
    {
        ...
        synchronized (mtx) {
            if (textureWeb != null) {
                //Log.i("webrender", "DRAWING WEB TEXTURE::" + textureWeb.getConfig());
                Util.transparent(textureWeb, 0xFFFFFFFF);
                GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, texture_id);
                GLUtils.texImage2D(GLES20.GL_TEXTURE_2D, 0, textureWeb, 0);
            }
        }
    }
    ...
    ///////////////////////////// ADDED FOR WEBVIEW /////////////////////////////
    public static void setTextureWeb(Bitmap texture) {
        synchronized (mtx) {
            //Log.i("VIDEO RENDER", "setTextureWeb");
            textureWeb = texture;
        }
    }

    public static void clearTextureWeb() {
        synchronized (mtx) {
            textureWeb = null;
            WebViewManager.setStopped();
        }
    }
}
